...
const FeesManager: React.FC<FeesManagerProps> = ({ user }) => {
  ...
  const handlePayFee = () => {
    ...
    if (paymentMode === 'GOOGLE_PAY' || paymentMode === 'PHONE_PE' || paymentMode === 'UPI') {
      ...
      const upiUrl = `upi://pay?pa=${masterUPI}&pn=${encodeURIComponent(schoolName)}&am=${targetFee.amount}&cu=INR&tn=${encodeURIComponent('School Fees: ' + targetFee.type)}`;
      
      // Redirect to UPI intent handler
    ...
    setTimeout(() => {
      ...
      createAuditLog(user, 'PAYMENT', 'Finance', `[${paymentMode}] Student ${user.name} processed payment ₹${targetFee.amount} via ${masterUPI}`);
  ...
  const handleOpenGlobalGateway = () => {
    if (schoolSettings?.school_payment_link) {
      window.open(schoolSettings.school_payment_link, '_blank');
    } else {
      // Fallback redirect to generic UPI selector if no link configured
      const masterUPI = 'www.ayazsurti2000-1@okaxis';
      window.location.href = `upi://pay?pa=${masterUPI}&pn=Deen-E-Islam%20School&cu=INR`;
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 relative">
      {showSuccess && (
        <div className="fixed top-24 right-8 z-[1000] animate-in slide-in-from-right-8 duration-500">
           <div className="bg-emerald-600 text-white px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-4 border border-emerald-500/50 backdrop-blur-xl">
              ...
              <div>
                 <p className="font-black text-xs uppercase tracking-[0.2em]">Transaction Sync</p>
                 <p className="text-[10px] font-bold text-emerald-100 uppercase tracking-widest mt-0.5">Payment Node Updated</p>
              </div>
           </div>
        </div>
      )}
           ...
           <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-1 shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-slate-100 dark:border-slate-800">
              <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
                 <div>
                    ...
                    <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mt-1">Direct Transfer to Merchant Account</p>
                 </div>
              ...
              <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
                 ...
                 <div className="space-y-4">
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Payment App Connection</label>
                 ...
                 <div className="animate-in fade-in slide-in-from-bottom-2">
                    <div className="p-6 bg-indigo-50 dark:bg-indigo-900/10 rounded-[2rem] border border-indigo-100 dark:border-indigo-800 flex items-center gap-4">
                       <div className="w-12 h-12 bg-white dark:bg-slate-800 rounded-2xl flex items-center justify-center text-indigo-600 shadow-sm"><ShieldCheck size={24}/></div>
                       <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Master UPI ID</p>
                          <p className="text-sm font-black text-slate-800 dark:text-white">{schoolSettings?.school_upi_id || 'www.ayazsurti2000-1@okaxis'}</p>
                       </div>
                    </div>

                    {(paymentMode === 'GOOGLE_PAY' || paymentMode === 'PHONE_PE' || paymentMode === 'UPI') && (
                       <div className="mt-6 p-8 text-center bg-emerald-50/50 dark:bg-emerald-950/20 rounded-2xl border border-emerald-100 dark:border-emerald-900">
                          <ExternalLink size={40} className="mx-auto text-emerald-600 mb-4 animate-bounce" />
                          <h5 className="font-black text-emerald-800 dark:text-emerald-400 uppercase text-xs">Mobile Deep Link</h5>
                          <p className="text-[10px] text-emerald-700/60 dark:text-emerald-500/60 font-medium uppercase mt-1">Opening secure payment selector on your device...</p>
                       </div>
                    )}
                 </div>
                 ...
                 >
                    {isProcessing ? <div className="w-5 h-5 border-3 border-white/30 border-t-white rounded-full animate-spin"></div> : (
                       <>
                          Launch {paymentMode.replace('_', ' ')} Gateway <ArrowRight size={18} />
                       </>
                    )}
                 </button>
              </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight leading-tight uppercase flex items-center gap-3">Fee Management <ShieldCheck className="text-indigo-600" /></h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">Centralized secure gateway for Deen-E-Islam School.</p>
        </div>

        <button 
          onClick={handleOpenGlobalGateway}
          className="px-8 py-4 bg-white dark:bg-slate-900 border-2 border-indigo-600 text-indigo-600 dark:text-indigo-400 font-black rounded-2xl shadow-xl flex items-center gap-3 hover:bg-indigo-600 hover:text-white transition-all group"
        >
          <Zap className="group-hover:animate-pulse" size={20} />
          <span className="uppercase text-xs tracking-widest">Pay with GPay / PhonePe</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800 p-8 rounded-[3rem] shadow-sm group hover:shadow-xl transition-all">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="font-black text-[10px] text-emerald-600 uppercase tracking-widest opacity-60">Verified</span>
          </div>
          ...
          <p className="text-emerald-600/70 dark:text-emerald-400/50 text-[10px] font-black mt-2 uppercase tracking-[0.2em]">Institutional Cleared</p>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800 p-8 rounded-[3rem] shadow-sm group hover:shadow-xl transition-all">
          ...
          <p className="text-amber-600/70 dark:text-amber-400/50 text-[10px] font-black mt-2 uppercase tracking-[0.2em]">Balance Due</p>
        </div>
        ...
        <div className="bg-indigo-600 p-8 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
          ...
          <h3 className="text-2xl font-black text-white tracking-tight relative z-10">Instant UPI</h3>
          <p className="text-indigo-200 text-[10px] font-black mt-2 uppercase tracking-[0.2em] relative z-10">9099711585</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
        <div className="p-10 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/30 dark:bg-slate-800/20">
          <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tight">Active Invoices</h3>
        </div>
                    ...
                    {fee.status === 'PAID' ? (
                      <div className="flex flex-col items-end gap-1">
                         <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-black text-[10px] uppercase tracking-widest">
                           <CheckCircle size={14} /> Synced Ledger
                         </div>
